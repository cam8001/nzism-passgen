...
class PasswordGenerator {
    ...
    while (word === null) {
      for (let i = 0; i < passphrase.length; i += 1) {
        ...
        if (
          ...
          w.length >= (this.config.min_password_length - nonWordCharacters + 1)
          // Is this word too long?
          && w.length <= (this.config.max_password_length - nonWordCharacters)
        ) {
          word = w;
          break;
        }
   ...
   */
  getWordList() {
    return niceware.generatePassphrase(this.config.password_byte_length);
  }
